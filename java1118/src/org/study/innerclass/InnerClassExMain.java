package org.study.innerclass;

import org.study.innerclass.InnerClassEx.InstanceClass2;

public class InnerClassExMain {
	

	public static void main(String[] args) {
		
		//인스턴스화(객체화)   new(객체생성, 객체주소생성)
		//클래스타입	// inner1 > 객체참조변수(객체의 주소값이 저장)
		InnerClassEx inner1 = new InnerClassEx();
									//생성자
		
		//내부클래스 생성
		inner1.new InstanceClass2();
//		new InnerClassEx().new InstanceClass2(); 위랑 같은것
			//내부 클래스 타입 선언						//내부 클래스 생성
		InnerClassEx.InstanceClass2 inC = inner1.new InstanceClass2();
		inC.num1 = 100;
		inC.m1();
// 		new InnerClassEx().new InstanceClass2();		
		
		//static 클래스			//static 클래스 생성 new 외부클래스.내부클래스();
		InnerClassEx.StaticClass2 s1 = new InnerClassEx.StaticClass2();
		InnerClassEx.StaticClass2.m2(); //static 매서드 구현
		
		s1.NUM = 20;
		s1.m2();
		
		//지역클래스
		InnerClassEx inC2 =  new InnerClassEx();
		inC2.localMethod();	

		
		
	}
	
	
	
		
}
